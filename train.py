from new_util import construct_graph, sample, get_roc_score, getMetapath, getNodeDict
import numpy as np
...
def Train(path, output_file, dim, window_size, iterations, batch_size, care_type, initial_lr, min_count, num_workers,
    ...
    for i in range(len(train_index)):
        ...
        # output_path = open(os.path.join(path, "output_path_ztdtz_first.txt"), "w")
        # print(hg)
        #---------------------------------------------------------------------------------------
        getMetapath(hg, TF_names, Target_names, Disease_names, num_walks_per_node, walk_length)

        m2v = Metapath2VecTrainer(path=path, output_file=output_file, dim=dim, window_size=window_size, iterations=iterations,
                                  batch_size=batch_size, care_type=care_type, initial_lr=initial_lr, min_count=min_count, num_workers=num_workers)

        m2v.train()

        
        # m2v = Metapath2VecTrainer(path = 'new_data/output_path_ztdtz_first.txt' ,output_file = 'TF_Target_Disease/output_first',
        #                           dim = 256 , window_size = 10 , iterations = 10, batch_size = 64, care_type = 0, initial_lr = 0.0001 , min_count = 1, num_workers = 5)
        # m2v.train()
