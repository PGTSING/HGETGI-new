...
class DataReader:
    ...
    def read_words(self, min_count):
        word_frequency = dict()
        for line in open(self.inputFileName, encoding="gbk"):
            line = line.split()
            if len(line) > 1:
        ...
        if self.hasnone == True:
            self.word2id['Cnone'] = wid
            # self.id2word[wid] = "Cnone"
            ...
            if len(line) > 1:
                ...
                if len(words) > 1:
                    ...
                    for i, u in enumerate(word_ids):
                        # if u == (len(self.data.word2id) -1 ) and self.data.hasnone==True:
                        #         continue
                        for j, v in enumerate(
                            ...
                            # if v == (len(self.data.word2id) - 1) and self.data.hasnone==True:
                            #     continue

                            if i == j:
                                continue
                            pair_catch.append((u, v, self.data.getNegatives(v,1)))
                    # print("pair_?catch",pair_catch)
                    return pair_catch
